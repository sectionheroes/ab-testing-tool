...
| Stats (MVP) | Fixed-horizon: Two-proportion z-test (CR), winsorized Welch t-test (RPV), Chi-Square SRM. `STATS_VERSION` in `lib/stats` wird bei jeder Formeländerung von Hand erhöht. | Einfach, prüfbar. Signifikanz erst nach geplanter Sample Size (Anti-Peeking). Sequential in Phase 2. |
| CI | `.github/workflows/ci.yml`: typecheck, lint, `pnpm test` (inkl. A/A-Monte-Carlo) und `pnpm test:db` (Postgres-Service-Container) bei jedem PR | „Pflicht vor jedem PR" in einer Doku ist keine Durchsetzung. Die DB-Tests laufen je Fall in einer zurückgerollten Transaktion und löschen nie eine Zeile. |
