...
- Scaffold aus dem React-Router-Template. **Zwei UI-Welten (ADR-0029):** `/app/*` = Polaris Web Components (`<s-page>`, `<s-section>`, `<s-text-field>`, `<s-button variant="secondary">`, `<s-badge>`, `<s-banner>`, `<s-link>`) aus `polaris.js` + `app-bridge.js`, beide nur auf `/app/*` in `root.tsx` injiziert, kein `app.css`/Tailwind dort; Typen über `@shopify/polaris-types` (devDependency, `tsconfig` `types`). Kein `@shopify/polaris` in `package.json`. Alles andere DESIGN.md. Lint-Regel `app/ui-split.test.ts` (läuft in `pnpm test`): `app/routes/app.*` ohne `app/components`-Imports, ohne `className`, ohne `variant="primary"`; kein `<s-*>` außerhalb.
...
- Install-Flow: eigene `ShopSessionStorage` (Shop-Row statt Template-`Session`-Tabelle, Token + Session AES-256-GCM via `TOKEN_ENCRYPTION_KEY`), ALLOWLISTED→ACTIVE, sonst PENDING; Reinstall nach UNINSTALLED → ACTIVE nur wenn `activatedAt` gesetzt. `afterAuth` liest `shop { name ianaTimezone }`. `/app`: PENDING = Aktivierungscode (timingSafeEqual, 5 Versuche/15 min) in `<s-text-field>` + nativem `<Form>` (`<s-button type="submit">` submittet, Enter im Feld nicht), falscher Code → `<s-banner tone="critical">`, richtiger → Redirect auf ACTIVE; ACTIVE = `<s-badge tone="success">` + Theme-Editor-Deep-Link (`<s-button href target="_top">`, öffnet den Editor im App-Embeds-Kontext, Toast „App embed does not exist“ bis WP3) + Kontakt. 10 Unit-Tests für die Storage-Übergänge.
...
- Polaris-Web-Components in React 18: boolesche Props nie als `{false}` übergeben (wird zu `loading="false"` = truthy) – `x || undefined`.
